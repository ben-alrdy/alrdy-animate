...
| `instant` | A **CSS-driven** first-paint entrance — the inline `@keyframes aa-load-in` plays it before the GSAP bundle loads, so above-the-fold heroes feel instant. The library builds **no** GSAP tween (appear/text features + reduced-motion pass skip it); the inline keyframe runs to completion on its own (it is **not** gated on init, so a fast init can't cut it short, and `backwards` fill reverts to the natural state after — no double-animation, no clobbered hover transform). Motion comes from `aa-animate` (element-level `fade`/`fade-up`/`blur`/`zoom`/`slide`/`rotate`). Add `alrdy-animate/loader` (a no-GSAP char/word splitter, ~0.9 KB gz) for `aa-delay`/`aa-duration`/`aa-ease` per-element timing and instant per-character `text-*` cascades (`text-fade`/`-up`/…, `text-blur`, `text-scale`); line-based `text-*` (slide/tilt/oval/rotate) fall back to an element-level fade (lines need font metrics). Self-revealing (needs no `aa-timeout` net). **The keyframe + reveal rules live only in the inline `<head>` snippet** (must be present at first paint) — see the Instant-hero recipe; preload the hero webfont for text. |
...
| `slider` | `aa-slider` | `Draggable`, `InertiaPlugin` | Draggable carousel with optional autoplay. (`ScrollTrigger` is used only if loaded, to viewport-gate autoplay; not required.) |
| `marquee` | `aa-marquee` | `Draggable`, `InertiaPlugin` | Infinite-loop scroller. (`ScrollTrigger` is used only if loaded, for `scrub` scroll-velocity coupling; not required.) |
...
| `intensity` | `1` | Multiplier for every feature that reads `aa-intensity` (fade/rotate/slide translate, parallax depth, stack transforms, text-fade/blur offsets, hover-icon trail timing, marquee scrub sweep, nav hide-clearance, tabs scroll-pin range). `1` reproduces each feature's design baseline; `0.5` halves, `2` doubles. **Baselines by family**: fade-\*/rotate-up `2rem`, text-blur (all directions) `2rem` (all root-font-size relative); slide-\*/text-slide-\*/text-tilt-\* line masks + hover-bg-block `100%` of element or line (element-relative); parallax `±10%` of range; marquee scrub `±10vw`. Rem-based values track `:root { font-size }` and rebuild on breakpoint changes via `gsap.matchMedia` — so per-breakpoint stepped root sizes Just Work; a fluid `clamp()` on `:root` locks in at breakpoint entry. |
