...
| `load-instant` | A **CSS-driven** first-paint entrance — the inline `@keyframes aa-load-in` plays it before the GSAP bundle loads, so above-the-fold heroes feel instant. The library builds **no** GSAP tween (appear/text features + reduced-motion pass skip it); the end-of-init `aa-ready` flip detaches the CSS rule, leaving the element at its natural end-state (no double-animation). Motion comes from `aa-animate` (element-level `fade`/`fade-up`/`blur`/`zoom`/`slide`/`rotate`). Add `alrdy-animate/loader` (a no-GSAP char/word splitter, ~0.9 KB gz) for an `aa-delay` staircase and instant per-character `text-*` cascades (`text-fade`/`-up`/…, `text-blur`, `text-scale`); line-based `text-*` (slide/tilt/oval/rotate) fall back to an element-level fade (lines need font metrics). Self-revealing (needs no `aa-timeout` net). **The keyframe + reveal rules live only in the inline `<head>` snippet** (must be present at first paint) — see the Instant-hero recipe; preload the hero webfont for text. |
