...
| `modal` | `aa-modal-group` | (none) | Fixed-position dialogs. The feature scans `[aa-modal-group]` (the container wrapping the card(s) + backdrop); each card is marked `[aa-modal-name]`, opened by `[aa-modal-target="<name>"]`, closed by `[aa-modal-close]` / `[aa-modal-backdrop]`. The lib writes `aa-modal-status` on each card and `aa-modal-group-status` on the container (`"active"` / `"not-active"`). |
