...
Preset elements are indistinguishable from hand-authored ones during the reduced-motion / `optimizeMobile` fade-fallback pass. With either active:

- `text-*` and appear (`fade-*`, `zoom-*`, `slide-*`, `blur-in`, `rotate-*`) presets collapse to a simple opacity fade.
- `parallax` presets are skipped entirely (parallax is dropped under both modes).
- Any preset element targeted by the fade pass uses the same timing as hand-authored elements.

No extra wiring required — the preset map is consulted by the fade-fallback pass alongside real attribute lookups.
