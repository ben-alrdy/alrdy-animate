...
Add the following to your site's custom code. Put the `<link>` and `<script defer>` tags in the **head**; put the `init()` call before `</body>`. Every dependency loads in parallel and executes in document order before the body finishes parsing.
...
<!-- Before </body>: wait for DOMContentLoaded so all defer scripts have executed -->
<script>
...
**Why `defer`** — without it, every `<script>` blocks HTML parsing, which makes the whole page wait for each download in series. With `defer`, all scripts download in parallel while the parser sails through the body; they then execute in document order at the end of parsing. This is faster on every connection, and it's a hard requirement for the [load-fallback recipe](/recipes/load-fallback/) to work on slow networks. The init call is wrapped in `DOMContentLoaded` because inline `<script>` tags in body run *during* parsing, before deferred head scripts have executed — without the wrapper, `AlrdyAnimate` would be undefined.
