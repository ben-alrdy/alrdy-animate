...
| `options` | Live readonly snapshot of the resolved options after `init()` — every `InitOptions` field with defaults filled in, plus `reducedMotion` / `optimizeMobile` collapsed to the booleans the lib actually uses internally. Reads always reflect the most recent init. |

`init` / `destroy` / `refresh` are the lifecycle hooks for Barba, View Transitions, or any other page-transition library — see the [Webflow + Barba recipe](/recipes/webflow-barba/) for a full example. `ready` + `options` + `onResize` are the interop surface for project-specific GSAP scripts that load alongside the lib (custom navs, sliders, scroll-driven scenes) — see the [Webflow guide](/getting-started/webflow/#global-gsap-code-runs-on-every-page) for the recommended pattern.
