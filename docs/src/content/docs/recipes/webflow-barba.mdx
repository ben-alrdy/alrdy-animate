...
- **Hero text animates on first load, just renders on subsequent navs.**
  Above-the-fold static elements use `aa-trigger="load"`. On first page load
  it animates (or the [CSS fallback](/recipes/load-fallback/) plays if the
  bundle is slow). On Barba navigations the `load` trigger is a no-op —
  the element renders in its natural state once `init()` flips its
...
- **Cards animate on every navigation via `event:enter`.** Combined
  `aa-trigger="load event:enter"` on the grid means: first load fires via
  `load` (immediate), every subsequent Barba nav fires via the `aa:trigger`
  event dispatched from the leave timeline, perfectly in sync with the
  visual peak. No double-fire, no replay-while-hidden.
...
- `aa-trigger="load event:enter"` on the cards — combined trigger. On the
  first page load in this session, `load` fires immediately (or the
  [CSS fallback](/recipes/load-fallback/) plays if the bundle is slow). On
  every subsequent Barba navigation, `load` is a no-op (first-init-only)
  and `event:enter` waits for an `aa:trigger` event with
...
- `aa-trigger="load"` (alone) on the headline + paragraph — animates on
...
barba.hooks.beforeEnter((data) => {
  ...
  // gates on initPromise to make sure listeners are attached.
  alrdyDestroy()
...
barba.init({
  ...
  transitions: [{
    ...
    async once(data) {
      ...
      // wait on the same initPromise instead of starting a second init.
      await initPromise
...
function runPageLeaveAnimation(current, next) {
  ...
  // Gated on initPromise in case init hadn't finished by this point.
  tl.call(() => {
