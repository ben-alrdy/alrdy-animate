...
- **Content animations are event-triggered, not viewport-triggered.** Boxes
  use `aa-trigger="event:enter"`, dispatched from a `tl.call` inside the
  leave timeline at the right beat. Without this, the boxes auto-fire while
  still hidden behind the wrapper.
...
<body data-barba="wrapper">
  ...
  <main data-barba="container" data-barba-namespace="home">
    ...
    <section class="page">
      <h1 class="page__hero">Headline</h1>
      <p class="page__lede">Subline copy</p>

      <div
        class="card-grid"
        aa-animate="fade-up"
        aa-stagger="0.1"
        aa-trigger="event:enter"
      >
...
- `aa-trigger="event:enter"` — the animation is paused until an
  `aa:trigger` event with `detail.name === 'enter'` is dispatched on the
  grid. Keeps the cards from auto-firing while they sit invisibly behind
  the leave-timeline wrapper.
...
Add only the GSAP plugins your features use — `init({ debug: true })`
...
| `aa-trigger="event:enter"`           | Pause an animation until an `aa:trigger` event fires.      |
