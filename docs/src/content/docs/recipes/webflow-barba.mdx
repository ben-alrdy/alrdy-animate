...
`init()` (with a `root` option), `destroy()` (with a `keepGlobals` option),
and `refresh()` so you can wire them into whatever transition library suits
your stack. This recipe shows the Webflow flavour: [Barba.js](https://barba.js.org/)
intercepts link clicks, runs an animated transition, and we hook the lib
into Barba's `beforeEnter` so the new page's `aa-*` elements get scanned
during the transition instead of after it.
...
| `destroy({ keepGlobals: true })`     | Tear down per-page state; keep Lenis, scroll-state alive.  |
...
- **Drop `initLenis()`.** `init({ smoothScroll: true })` creates Lenis and
  ...
  working unchanged.
- **Drop the `afterLeave` ScrollTrigger kill.** `destroy({ keepGlobals: true })`
  in `beforeEnter` reverts every `gsap.matchMedia()` context the lib
  created, which kills its own ScrollTriggers and restores any inline
  styles. The global Lenis/scroll-state observers stay alive.
