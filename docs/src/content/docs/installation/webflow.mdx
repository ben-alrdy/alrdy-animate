...
For above-the-fold hero copy, `aa-trigger="load-instant"` paints its entrance on the first frame as pure CSS — before GSAP loads — so there's no wait for the bundle at all. It needs its own inline `<head>` block; see the [Instant hero entrances recipe](/recipes/instant-hero/).
:::
