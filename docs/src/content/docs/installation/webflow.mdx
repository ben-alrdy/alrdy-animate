...
:::note[Which inline snippet do you need?]
The block below is for **GSAP `load` / `load-once` heroes** — it includes the slow-network `aa-fallback` staircase. If your above-the-fold heroes use **`aa-trigger="instant"`** instead (CSS entrance, no bundle wait), use the leaner block from the **[Instant hero recipe](/recipes/instant-hero/)** — it drops the `aa-fallback` staircase and adds the instant CSS + loader. Both share the *same* FOUC guard, so if you use both kinds of hero you merge them into one block (one guard, concatenate the rest).
:::
...
:::tip[Want the hero to animate *instantly*?]
For above-the-fold hero copy, `aa-trigger="instant"` paints its entrance on the first frame as pure CSS — before GSAP loads — so there's no wait for the bundle at all. It needs its own inline `<head>` block; see the [Instant hero entrances recipe](/recipes/instant-hero/).
:::
