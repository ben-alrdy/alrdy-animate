...
Paste this into **Project Settings → Custom Code → Head Code**. It loads GSAP, Lenis, and alrdy-animate in parallel via `defer`, preceded by an inline FOUC guard + slow-network fallback that's active from the first paint — so animated content is never left invisible while the bundle downloads.
