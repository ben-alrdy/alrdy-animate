...
**Why the stylesheet is non-blocking.** `media="print"` tells the browser to fetch the file at low priority without holding up the first paint; `onload="this.media='all'"` swaps it to screen styles the moment it lands, and the `<noscript>` copy keeps it working with JS disabled. This is only safe because the FOUC guard, fallback keyframe, and timeout rules are inlined in the safety snippet above — the external file then carries just helper classes and interactive-component defaults, none of which the initial render needs. One caveat: a marquee, tabs, or hover-text component placed above the fold can flash unstyled for the moment before the file lands; if that matters on a given page, inline the whole `dist/alrdy-animate.css` into the safety snippet instead.
