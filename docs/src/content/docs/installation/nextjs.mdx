...
```tsx
<h1 aa-animate="text-slide-up" aa-split="lines mask" aa-trigger="page-enter">
  Headline
</h1>
```

**Why `page-enter` and not `load`?** In Next.js App Router, the `AlrdyInit` component lives in the root layout and persists across route changes. If you re-init on `usePathname()` change (see [SPA route changes](#spa-route-changes) below), the second/third/Nth call to `init()` is the user's first *visual* arrival on that page — but `load` treats it as a subsequent init and skips. `page-enter` fires on every `init()` cycle so back/forward nav and `<Link>` clicks all animate the hero. See [aa-trigger](/animations/utilities/aa-trigger/) for the full comparison.
...
For elements that should animate **every time** the user lands on the route (cold load, forward `<Link>` click, browser back/forward), use `aa-trigger="page-enter"` instead of `aa-trigger="load"`.
