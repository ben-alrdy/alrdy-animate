...
This guide is for Webflow sites that use [Barba.js](https://barba.js.org/) for SPA-style page transitions. The script setup is the same as the [simple Webflow guide](/installation/webflow/) plus the Barba script — what changes is the init wiring: `init()` runs in `beforeEnter` scoped to the new container, and `destroy()` tears down per-page state without disturbing Lenis.
...
Same as the [simple Webflow guide](/installation/webflow/#1-site-wide-head-code), with one extra script — `@barba/core` — added at the end:

```html
<!-- (paste both blocks from the simple guide here: the inline safety snippet,
     then your chosen CSS + GSAP/Lenis/alrdy-animate variant) -->
