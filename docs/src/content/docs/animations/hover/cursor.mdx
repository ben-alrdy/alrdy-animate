...
| ---------------------- | -------------------------------------------------------------------------------------- |
| `aa-cursor`            | Cursor wrapper. Optional value names the cursor so triggers can target it; multiple cursors with the same name are tolerated but only the first wins (warns in `debug`). |
| `aa-cursor-trigger`    | Trigger element. The value selects which `[aa-cursor]` to use; matched by name (both empty = default pair). Triggers whose name has no matching cursor are silently ignored. |
