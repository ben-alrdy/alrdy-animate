...
| `aa-color`    | `currentColor` | Colour applied to the cloned SVG(s) only — the original keeps the inherited colour. Use to make the slide-in clone stand out against a coloured hover background (e.g. `aa-color="#ffffff"` on an icon inside a `curve` button filling black). Requires the SVG to use `stroke="currentColor"` or `fill="currentColor"`. |
...
| `aa-intensity` | `1`           | multiplier on the base 50 ms lag between each successive icon starting its slide — `2` doubles the gap (slower trail), `0.5` halves it (tighter chain) |
...
<Demo caption={{ "aa-hover": "icon-* triple", "aa-intensity": "0.5" }}>
  <div data-demo-stage>
    <div data-spacer></div>
    <div class="hover-row">
      <a href="#" class="hover-cta" aa-hover="icon-up-right triple" aa-intensity="0.5">
        <span>Up-right (triple)</span>
        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7 17 17 7"/><path d="M8 7h9v9"/></svg>
      </a>
      <a href="#" class="hover-cta" aa-hover="icon-right triple reverse" aa-intensity="0.5">
