...
<Demo caption={[{ label: "nav", attrs: { "aa-nav": "hide change", "aa-ease": "osmo" } }, { label: "current pill", attrs: { "aa-nav-current-indicator": "", "aa-duration": "0.45", "aa-ease": "power3.out" } }]}>
  <div data-demo-stage>
    ...
    <section id="bar-demo-4" class="aa-nav-demo-section">
      <h3>contact</h3>
      <p>The body picks up <code>aa-scroll-direction</code> and <code>aa-scroll-started</code> attributes — the hide rule is just a CSS transform keyed on those.</p>
    </section>
  </div>
</Demo>
...
<style>{`
  ...
  .aa-nav-demo {
    ...
    border-radius: 999px;
    margin: 0;
    /* No transform here. The lib uses CSS animation (not transition) for the
       hide, so user transitions on the same element don't clobber it. */
