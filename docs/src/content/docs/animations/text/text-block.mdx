...
`aa-split` is locked to `lines`: the reveal places one bar over each line, so chars/words don't apply. Any value you set is ignored.
...
| `aa-split`    | `lines` (locked) | Cannot be overridden — one bar is placed over each line.                               |
