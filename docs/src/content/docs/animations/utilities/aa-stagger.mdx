...
If you omit the second number, the per-line offset defaults to the `lines` stagger (0.1s), independent of the unit. So `aa-split="lines-chars"` with no `aa-stagger` staggers chars 0.02s apart and cascades lines 0.1s apart, and `aa-stagger="0.05"` on a line-grouped split keeps the 0.1s line offset while setting chars to 0.05s. Pass a second number to override it: `aa-stagger="0.05 0.3"`.
