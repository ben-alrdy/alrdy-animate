...
| `load-instant`  | A **CSS-driven** entrance that paints on the first frame, *before* GSAP loads — the hero feels instant. The library builds no GSAP tween (it's owned by an inline keyframe). Element-level motion from `aa-animate`; add the loader for `aa-delay` staircases and per-character `text-*` cascades. See [Instant hero entrances](/recipes/instant-hero/). |
