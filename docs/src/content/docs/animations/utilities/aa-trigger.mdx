...
`load` and `load-once` animations also pick up the global [`loadDelay`](/initialization/init-options/) init option (default `0.1s`), which is added on top of any per-element `aa-delay` so the entrance plays a beat after `init()` settles. Set `loadDelay: 0` to opt out globally.
