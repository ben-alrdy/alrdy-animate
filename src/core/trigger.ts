...
export type TriggerKind = 'scroll' | 'event' | 'click' | 'load-once' | 'load' | 'lcp' | 'load-instant'
...
function parseOne(token: string): ParsedTrigger | null {
  ...
  // `load-instant` is a CSS-driven first-paint entrance: the inline `<head>`
  ...
  // staggers; without it, a `load-instant` element gets an element-level fade.
  if (token === 'load-instant') return { kind: 'load-instant' }
